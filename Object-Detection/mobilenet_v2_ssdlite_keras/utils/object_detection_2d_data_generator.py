...
class DataGenerator:
    def __init__(self,
        ...
        if not filenames is None:
            if isinstance(filenames, (list, tuple)):
                ...
                raise ValueError(
                    "`filenames` must be either a Python list/tuple or a string representing \
                    a filepath (to a pickled or text file). The value you passed is neither of the two.")
        ...
        if not labels is None:
            if isinstance(labels, str):
                ...
                raise ValueError(
                    "`labels` must be either a Python list/tuple or a string representing the \
                    path to a pickled file containing a list/tuple. The value you passed is neither of the two.")
        else:
            self.labels = None
        ...
        if not image_ids is None:
            if isinstance(image_ids, str):
                ...
                raise ValueError(
                    "`image_ids` must be either a Python list/tuple or a string representing the \
                    path to a pickled file containing a list/tuple. The value you passed is neither of the two.")
        else:
            self.image_ids = None
    ...
    def parse_csv(self,
        ...
        Returns:
            None by default, optionally lists for whichever are available of images, image filenames, labels,
            and image IDs.
        """
    ...
    def parse_json(self,
        ...
        Arguments:
            ...
            ground_truth_available (bool, optional): Set `True` if the annotations files contain ground truth
            information.
            include_classes (list, optional): Either 'all' or a list of integers containing the class
                IDs that are to be included in the dataset. If 'all', all ground truth boxes will be included
                in the dataset.
            ret (bool, optional): Whether or not to return the outputs of the parser.
            verbose (bool, optional): If `True`, prints out the progress for operations that may take a bit longer.
        Returns:
            None by default, optionally lists for whichever are available of images, image filenames,
        ...
        if self.labels is None:
            if any([ret in returns for ret in
                    ['original_labels', 'processed_labels', 'encoded_labels', 'matched_anchors',
                     'evaluation-neutral']]):
                warnings.warn(
                    "Since no labels were given, none of 'original_labels', \
                    'processed_labels', 'evaluation-neutral', 'encoded_labels', and 'matched_anchors' " +
            ...
            if any([ret in returns for ret in ['encoded_labels', 'matched_anchors']]):
                warnings.warn(
                    "Since no label encoder was given, \
                    'encoded_labels' and 'matched_anchors' aren't possible returns, " +
                ...
                if not (self.labels is None):
                    ...
                    if np.any(batch_y[i][:, xmax] - batch_y[i][:, xmin] <= 0) or np.any(
                        ...
                        if degenerate_box_handling == 'warn':
                            warnings.warn(
                                "Detected degenerate ground truth bounding boxes for batch item {} \
                                with bounding boxes {}, ".format(
                                    i, batch_y[i]) +
                                ...
                                or that any image transformations you may apply might \
                                result in degenerate ground truth boxes, \
