...
class SSDLoss:
    ...
    def compute_loss(self, y_true, y_pred):
        ...
        self.neg_pos_ratio = tf.convert_to_tensor(self.neg_pos_ratio)
        # self.n_neg_min = tf.constant(self.n_neg_min)
        # self.alpha = tf.constant(self.alpha)
        self.n_neg_min = tf.convert_to_tensor(self.n_neg_min)
        self.alpha = tf.convert_to_tensor(self.alpha)
        ...
        classification_loss = tf.cast(
            self.log_loss(y_true[:, :, :-12], y_pred[:, :, :-12]), tf.float32)
        print(classification_loss)  # Output shape: (batch_size, n_boxes)
        ...
        # What's the point of `n_neg_losses`? For the next step, which will be to compute
        # which negative boxes enter the classification loss, we don't just want to know
        # how many negative ground truth boxes there are, but for how many of those there
        # actually is a positive (i.e. non-zero) loss. This is necessary because `tf.nn.top-k()`
        # in the function below will pick the top k boxes with the highest losses no matter what,
        # even if it receives a vector where all losses are zero. In the unlikely event that all
        # negative classification losses ARE actually zero though, this behavior might lead to
        # `tf.nn.top-k()` returning the indices of positive boxes, leading to an incorrect negative
        # classification loss computation, and hence an incorrect overall loss computation.
        ...
        def f2():
            # Now we'll identify the top-k (where k == `n_negative_keep`) boxes with
            # the highest confidence loss that belong to the background class in the
            # ground truth data. Note that this doesn't necessarily mean that the model
            # predicted the wrong class for those boxes, it just means that the loss for
            # those boxes is the highest.
